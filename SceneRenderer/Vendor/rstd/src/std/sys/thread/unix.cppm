...
#ifdef RSTD_OS_APPLE
        // macOS pthread_setname_np is 1-arg and only sets the *calling*
        // thread's name. The Linux 2-arg form lets us set any thread's
        // name; we approximate by only honouring set_name when invoked
        // from the target thread itself, which is the common case.
        ::pthread_setname_np((char const*)name.p);
#else
